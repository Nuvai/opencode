...
Use Claude models through Azure's Anthropic endpoint. This provider is recommended as a default option for accessing latest Claude models.

1. Head over to the [Azure portal](https://portal.azure.com/) and create an **Azure Anthropic** resource. You'll need:
   - **Resource name**: This becomes part of your API endpoint (`https://RESOURCE_NAME.openai.azure.com/anthropic/v1`)
   - **API key**: Your Anthropic API key from your Azure resource

   :::tip
   The Azure Anthropic resource provides access to Claude models through the standard Anthropic API format.
   :::

2. Get your API key:
   - Go to your Azure Anthropic resource in the Azure portal
   - Navigate to **Keys and Endpoint** section
   - Copy either **Key 1** or **Key 2**

3. Run the `/connect` command and search for **Azure Anthropic**.
...
5. Enter your Azure resource name when prompted. This will be stored in your config for future use.

    ```txt
    ┌ Enter your Azure resource name (e.g., my-openai-resource)
    │
    │
    └ enter
    ```
   ...
   Alternatively, you can set it as an environment variable:
    ...
    ```bash
    AZURE_ANTHROPIC_RESOURCE_NAME=XXX opencode
    ```

    Or add it to your bash profile:

    ```bash title="~/.bash_profile"
    export AZURE_ANTHROPIC_RESOURCE_NAME=XXX
    ```

6. Run the `/models` command to select a Claude model.
   ...
   Available models:
   - **Claude Sonnet 4** - Latest flagship model with extended thinking
...
Azure OpenAI Service now supports Anthropic Claude models through their platform. This allows you to use Claude models with Azure's enterprise features and compliance.

1. Head over to the [Azure portal](https://portal.azure.com/) and create an **Azure OpenAI** resource. You'll need:
   - **Resource name**: This becomes part of your API endpoint (`https://{resource-name}.openai.azure.com/anthropic/v1`)
   - **API key**: Either `KEY 1` or `KEY 2` from your resource

2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a Claude model (e.g., `claude-sonnet-4-20250514`, `claude-3-5-sonnet-20241022`).
...
5. Enter your resource name when prompted, or set it as an environment variable:

   ```bash
   AZURE_ANTHROPIC_RESOURCE_NAME=my-resource opencode
   ```

   Or add it to your bash profile:

   ```bash title="~/.bash_profile"
   export AZURE_ANTHROPIC_RESOURCE_NAME=my-resource
   ```

6. Run the `/models` command to select your deployed Claude model.
...
You can also configure Azure Anthropic directly in your config file:
...
[LiteLLM](https://litellm.ai/) is an OpenAI-compatible proxy that lets you access 100+ LLM providers through a unified API. OpenCode auto-detects running LiteLLM servers and dynamically discovers available models.

#### Local Proxy (No Auth)

For a local LiteLLM proxy running on the default port, OpenCode will automatically detect it:

```bash
# Start LiteLLM proxy
litellm --model gpt-3.5-turbo

# OpenCode will auto-detect the server
opencode
```

Run `/models` to see the discovered models.
...
```bash
# Custom URL (default: http://localhost:4000)
LITELLM_BASE_URL=http://localhost:8000 opencode

# With API key for authenticated proxies
LITELLM_API_KEY=sk-xxx LITELLM_BASE_URL=https://litellm.example.com opencode
```
...
export LITELLM_API_KEY=sk-xxx  # Optional for local proxies
```

#### Using /connect

For remote LiteLLM proxies that require authentication:

1. Run the `/connect` command and search for **LiteLLM**.
...
3. Set the base URL if not using the default:

   ```bash
   LITELLM_BASE_URL=https://litellm.example.com opencode
   ```
...
For persistent configuration or custom models:
...
You can also define custom models if auto-discovery doesn't work:
...
:::tip
LiteLLM models are auto-detected from the `/v1/models` endpoint. Custom models in your config will be merged with discovered models.
:::
